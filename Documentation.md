...
Sprites can be identified in one of two ways: its spriteID and its spriteName. The spriteID will simply be the order in which each sprite was added, starting from 1. Eg sprite ID 3 is the third sprite in the sprite set. The spriteName is a char string that is set to the file name of the sprite without its extension. (Built-in functionality to alter the name of a sprite and altering the ID/ordering of a sprite will be added in a future version, likely v0.05)
