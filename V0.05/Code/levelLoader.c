...
int loadLevelData(World *gameWorld, int level)
{
	...
	while (endOfFile == 0 && i < 8000)
	{
		...
		else if (charBuffer[0] == '>')
		{
			...
			while (commentLength < 80 && charBuffer[0] != '/')
			{
				objectPosition = ftell(fPtr);
				getNextArg(fPtr, charBuffer, 1);
				commentLength++;
			}
		...
		// This is incredibly hacky but basically I do this incase only one entry is malformed, stops at '////'
		int temp[16];

		readMultipleIntArgs(fPtr, temp, 16, 6);
...
int loadLevelFlag(World *gameWorld, FILE *fPtr)
{
	...
	if (strcmp(buffer, "SET-BG-TRIGGER") == 0)
	{
		...
		int returnMsg = readMultipleIntArgs(fPtr, args, 8, 6);
	...
	if (strcmp(buffer, "SET-CAMBOX") == 0)
	{
		...
		int returnMsg = readMultipleIntArgs(fPtr, args, 4, 5);
	...
	if (strcmp(buffer, "START-CAMPOS") == 0)
	{
		...
		int returnMsg = readMultipleIntArgs(fPtr, args, 2, 6);
...
int loadObject(World *gameWorld, FILE *fPtr, int xOffset, int yOffset)
{
	...
	if (readMultipleIntArgs(fPtr, args, 7, 5) == -1)
	{
		return -1;
	}
	...
	addedObject = AddObject(gameWorld, coreArgs[0], coreArgs[1] + xOffset, coreArgs[2] + yOffset, args[0], args[1], args[2], args[3], args[4], args[5], args[6]);	
