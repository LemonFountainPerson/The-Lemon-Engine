...
int smoothSizeChangeTo(Object *input, float desiredSize, float rate)
{
	...
	if (fabs(input->ObjectDisplay->size - desiredSize) < 0.001)
	{
		input->ObjectDisplay->size = desiredSize;
		return EXECUTION_UNNECESSARY;
	}
...
bool MouseOverlappingBox(Object *input, Camera inputCam)
{
	...
	mouseBox.xSize = 1;
	mouseBox.ySize = 1;
	mouseBox.xPos = getMouseXZoom(inputCam);
	mouseBox.yPos = getMouseYZoom(inputCam);

	if (input->layer != HUD)
	{
		mouseBox.xPos += inputCam.CameraX;
		mouseBox.yPos += inputCam.CameraY;
	}
