...
int RunLemonEngine(void)
{
    ...
    while(GameWorld.GameState != CLOSE_GAME)
    {
	    ...
	    ScreenData.FrameTimer += timeElapsed;
...
int getExternalInput(World *GameWorld)
{
	...
	keyboard[LMN_TEXT_SKIP] = keyboard[LMN_INTERACT2] || keyboard[LMN_SPACE] || MouseInput.RightButton;
...
void MasterControls(World *GameWorld, PlayerData *player)
{
	...
	if (keyboard['L'])
	{
		if (keyboard['1'] == 1)
		{
			...
			{
				DebugSettings.DebugTextInfoPreset = 16;
			}
		}
		...
		if (keyboard['2'] == 1)
		{
			...
			DebugSettings.DebugTextInfoPreset = (DebugSettings.DebugTextInfoPreset + 1) % 17;
		}
