...
int StartCutscene(CutsceneID inputID, World *GameWorld)
{
	...
	{
		GameWorld->Player.PlayerPtr->State = ACTOR;
	}
	...
	default:
		...
		if (StartCutsceneFromFile(fileName, GameWorld) == LEMON_SUCCESS)
		{
			GameWorld->CurrentCutscene = inputID;
		}
		break;
...
int StartCutsceneFromFile(const char sceneName[], World *GameWorld)
{
	if (GameWorld == NULL || sceneName == NULL)
	{
		return MISSING_DATA;
	}
	...
	while (!endOfFile(fPtr, readString))
	{
...
SceneAction* loadSceneAction(char inputString[MAX_LEN], char textBoxString[MAX_LEN], World *GameWorld, FILE *fPtr)
{
	...
	if (strcmp(inputString, "SAYTEXTOPTION:") == 0)
	{
		...
		int numberOfOptions = clamp(getNextArgInt(fPtr), 0, MAX_TEXT_OPTIONS);

		char options[MAX_TEXT_OPTIONS][OPTION_TEXT_MAX_LEN] = {0};
		int sceneID[MAX_TEXT_OPTIONS] = {0};

		for (int i = 0; i < numberOfOptions && i < MAX_TEXT_OPTIONS; i++)		// GCC complains if this loop isnt guarded with 'i < MAX_TEXT_OPTIONS'
		{
		...
		case 1:
			{
				SayTextOption(textBoxString, inputString, Preset, GameWorld, 1, 
						options[0], &StartCutscene, sceneID[0]);
			} break;
		...
		case 2:
			{
				SayTextOption(textBoxString, inputString, Preset, GameWorld, 2, 
						options[0], &StartCutscene, sceneID[0], 
						options[1], &StartCutscene, sceneID[1]);
			} break;
		...
		case 3:
			{
				SayTextOption(textBoxString, inputString, Preset, GameWorld, 3, 
						options[0], &StartCutscene, sceneID[0], 
						options[1], &StartCutscene, sceneID[1],
						options[2], &StartCutscene, sceneID[2]);
			} break;
		...
		case 4:
			{
				SayTextOption(textBoxString, inputString, Preset, GameWorld, 4, 
						...
						options[1], &StartCutscene, sceneID[1],
						options[2], &StartCutscene, sceneID[2],
						options[3], &StartCutscene, sceneID[3]);
			} break;
	...
	{
		getNextArg(fPtr, inputString, MAX_LEN);
		return SetActorPosition(inputString, getNextArgFloat(fPtr), getNextArgFloat(fPtr), GameWorld);
	}
	else if (strcmp(inputString, "MOVEACTOR:") == 0)
	{
		getNextArg(fPtr, inputString, MAX_LEN);
		return MoveActor(inputString, getNextArgFloat(fPtr), getNextArgFloat(fPtr), getNextArgInt(fPtr), GameWorld);
	}
	else if (strcmp(inputString, "MOVEACTORX:") == 0)
	{
		getNextArg(fPtr, inputString, MAX_LEN);
		return MoveActorX(inputString, getNextArgFloat(fPtr), getNextArgInt(fPtr), GameWorld);
	}
	else if (strcmp(inputString, "MOVEACTORY:") == 0)
	{
		getNextArg(fPtr, inputString, MAX_LEN);
		return MoveActorY(inputString, getNextArgFloat(fPtr), getNextArgInt(fPtr), GameWorld);
	}
	...
	{
		getNextArg(fPtr, inputString, MAX_LEN);
		return CreateActor(inputString, getNextArgInt(fPtr), getNextArgInt(fPtr), getNextArgInt(fPtr), GameWorld);
	}
	...
	else if (strcmp(inputString, "PLAYSOUND:") == 0)
	{
		getNextArg(fPtr, inputString, MAX_LEN);
		getNextArg(fPtr, textBoxString, MAX_LEN);
		return SceneAction_PlaySoundRepeat(inputString, textBoxString, getNextArgInt(fPtr), getNextArgFloat(fPtr), getNextArgInt(fPtr), GameWorld);
	}
	else if (strcmp(inputString, "SETCHANNELVOL:") == 0)
	{
		return SceneAction_SetSoundChannelVolume(getNextArgInt(fPtr), getNextArgFloat(fPtr), GameWorld);
	}
	else if (strcmp(inputString, "FADECHANNELVOL:") == 0)
	{
		return SceneAction_FadeSoundChannel(getNextArgInt(fPtr), getNextArgFloat(fPtr), getNextArgInt(fPtr), GameWorld);
	}
	else if (strcmp(inputString, "SETCAMERAPOS:") == 0)
	{
		return SceneAction_SetCameraPosition(getNextArgFloat(fPtr), getNextArgFloat(fPtr), GameWorld);
	}
	else if (strcmp(inputString, "MOVECAMERA:") == 0)
	{
		return SceneAction_MoveCamera(getNextArgFloat(fPtr), getNextArgFloat(fPtr), getNextArgInt(fPtr), GameWorld);
	}
	else if (!strcmp(inputString, "SMOOTHMOVECAMERATO:") || !strcmp(inputString, "MOVECAMERATO:"))
	{
		return SceneAction_MoveCameraSmooth(getNextArgFloat(fPtr), getNextArgFloat(fPtr), getNextArgFloat(fPtr), GameWorld);
	}
	else if (!strcmp(inputString, "SETCAMERAZOOM:") || !strcmp(inputString, "SETZOOM:"))
	{
		return SceneAction_SetZoom(getNextArgFloat(fPtr), getNextArgFloat(fPtr), GameWorld);
	}
	else if (!strcmp(inputString, "SETCAMERAZOOM:") || !strcmp(inputString, "SETZOOM:"))
	{
		return SceneAction_ChangeZoom(getNextArgFloat(fPtr), getNextArgFloat(fPtr), getNextArgInt(fPtr), GameWorld);
	}
...
int UpdateCutscene(World *GameWorld)
{
	...
	if ((GameWorld->SceneActionQueue == NULL && GameWorld->CurrentCutscene == NO_CUTSCENE))// || GameWorld->PlayingText == 1)
	{
...
int updateSceneActions(World *GameWorld)
{
	...
	{
		if (RunSceneAction(ActionPtr, GameWorld) == DATA_CLEARED || GameWorld->SceneActionQueue == NULL)
		{
			//printf("\nCLEARED\n");
			return DATA_CLEARED;
		}
...
int RunSceneAction(SceneAction *inputAction, World *GameWorld)
{
	...
	case SCENE_IF_EQUALS:
		{
			...
			int returnMsg = MISSING_DATA;

			if (GameFlags[branchData.variableIndex] == branchData.comparisonValue)
			{
				if (branchData.ifTrueString[0] != 0)
				{
					StartCutsceneFromFile(branchData.ifTrueString, GameWorld);
				}	
				else
				{
					returnMsg = StartCutscene(branchData.ifTrue, GameWorld);
				}
			}
			else if (branchData.elseBranchPresent)
			{
				...
				{
					StartCutsceneFromFile(branchData.ifFalseString, GameWorld);
				}	
				else
				{
					returnMsg = StartCutscene(branchData.ifFalse, GameWorld);
				}
			}
	...
	case SCENE_IF_LESS_THAN:
		{
			...
			if (GameFlags[branchData.variableIndex] < branchData.comparisonValue)
			{
				if (branchData.ifTrueString[0] != 0)
				{
					returnMsg = StartCutsceneFromFile(branchData.ifTrueString, GameWorld);
				}	
				else
				{
					returnMsg = StartCutscene(branchData.ifTrue, GameWorld);
			...
			else if (branchData.elseBranchPresent)
			{
				if (branchData.ifFalseString[0] != 0)
				{
					returnMsg = StartCutsceneFromFile(branchData.ifFalseString, GameWorld);
				}	
				else
				{
					returnMsg = StartCutscene(branchData.ifFalse, GameWorld);
	...
	case SCENE_RELEASE_ACTOR:
		{
			inputAction->repeatTimes = 0;
			if (inputAction->ActorObject != NULL && inputAction->ActorObject->State == ACTOR)
			{
				inputAction->ActorObject->State = DEFAULT;
			}
		} break;
