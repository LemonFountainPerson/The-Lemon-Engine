...
int DisplayObjectDebugInfo(Object *input, int objectNumber, bool goToMouse, Camera renderCamera)
{
	...
	case 4:
		snprintf(text, DEBUG_TEXT_MAX_LENGTH, 
			"ForwardVel: %.2f \nXVel: %.2f \nYVel: %.2f, \nBox Direction: %.2lf \nIn Air: %d", 
			inputBox->forwardVelocity, inputBox->xVelocity, inputBox->yVelocity, inputBox->direction * RADIAN_TO_DEGREE_PI, inputBox->inAir);
		break;

	case 5:
		snprintf(text, DEBUG_TEXT_MAX_LENGTH, 
			"SolidType: %d \nLayer: %d \nCollision Layer: %d \nObject List position: %d", 
			inputBox->solid, input->layer, inputBox->collideLayer, objectNumber);
		break;

	case 6:
		snprintf(text, DEBUG_TEXT_MAX_LENGTH, 
			"SolidType: %s \nLayer: %d \nCollision Layer: %d \nObject List position: %d", 
			ConvertSolidTypeToSolidName(inputBox->solid), input->layer, inputBox->collideLayer, objectNumber);
		break;
	...
	case 8:
		...
		{
			snprintf(text, DEBUG_TEXT_MAX_LENGTH, "Sprite ID: %d\nSprite name: %s", inputDisplay->currentSprite, inputDisplay->spriteBuffer->spriteName);
		}
	...
	case 9:
		...
		snprintf(text, DEBUG_TEXT_MAX_LENGTH, 
			"Sprite ID: %d \nSprite name: %s \nSprite Rendermode: %s \nRendermode Override: %s \nTransparency: %f", 
			inputDisplay->currentSprite, inputDisplay->spriteBuffer->spriteName, getRenderModeName(inputDisplay->spriteBuffer->RenderMode), 
			getRenderModeName(inputDisplay->RenderModeOverride), inputDisplay->transparency);
		break;
	...
	case 10:
		...
		snprintf(text, DEBUG_TEXT_MAX_LENGTH, "Sprite name: %s \nX Pos Offset: %f \nY Pos Offset: %f \nX Pixel Offset: %d \nY Pixel Offset: %d", 
				inputDisplay->spriteBuffer->spriteName, inputDisplay->spriteXOffset, inputDisplay->spriteYOffset, inputDisplay->pixelXOffset, inputDisplay->pixelYOffset);
		break;
	...
	case 12:
		...
		snprintf(text, DEBUG_TEXT_MAX_LENGTH, 
			...
			inputDisplay->frameBuffer->SpriteYOffset, (double)inputDisplay->frameBuffer->rotation * RADIAN_TO_DEGREE_PI);
		break;
	...
	case 13:
		...
		{
			snprintf(text, DEBUG_TEXT_MAX_LENGTH, 
			"Box Direction: %.2lf \nDisplay Direction: %.2lf \nNo animation currently Playing", 
			inputBox->direction * RADIAN_TO_DEGREE_PI, inputDisplay->direction * RADIAN_TO_DEGREE_PI);
		}
		else
		{
			snprintf(text, DEBUG_TEXT_MAX_LENGTH, 
			"Box Direction: %.2lf \nDisplay Direction: %.2lf \nFrame XPos Offset: %f \nFrame YPos Offset: %f \nFrame Rotation Offset: %.2lf", 
			inputBox->direction * RADIAN_TO_DEGREE_PI, inputDisplay->direction * RADIAN_TO_DEGREE_PI, 
			inputDisplay->frameBuffer->SpriteXOffset, inputDisplay->frameBuffer->SpriteYOffset, (double)inputDisplay->frameBuffer->rotation * RADIAN_TO_DEGREE_PI);
		}
		break;
	...
	case 14:
		snprintf(text, DEBUG_TEXT_MAX_LENGTH, 
			"Object State: %d \nCurrent Action: %d \nCurrent Interrupt: %d \nArg4: %d \nArg5: %d", 
			input->State, input->Action, input->Interrupt, input->arg4, input->arg5);
		break;
	...
	case 15:
		snprintf(text, DEBUG_TEXT_MAX_LENGTH, 
			"Arg1: %d \nArg2: %d \nArg3: %d \nArg4: %d \nArg5: %d", 
			input->arg1, input->arg2, input->arg3, input->arg4, input->arg5);
		break;
