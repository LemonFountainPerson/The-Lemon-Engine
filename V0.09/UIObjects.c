...
int UpdateCursor(Object *Cursor, World *GameWorld)
{
	...
	if (MouseInput.LeftButton == 1 && PlayerBox != NULL && Cursor->Interrupt == NO_INTERRUPT)
	{
	...
	else if (MouseInput.LeftButton == 0)
	{
		Cursor->Interrupt = NO_INTERRUPT;
	}
...
Object* AddOptionButton(const char spriteName[], int xPos, int yPos, Object *MenuController, World *GameWorld)
{
	...
	Object *createdOption = AddObjectWithParent(GameWorld, MenuController, UI_ELEMENT, xPos, yPos, 256, 40, OPTION_BUTTON, MenuController->arg3, 0, 0, 0);
...
int handleOptionPrompt(TextInstance *inputText, World *GameWorld)
{
	...
	if (inputText->boxPtr != NULL)
	{
		...
		int i = -1;
		while (!MouseOverlappingBox(&optionObj, GameWorld->MainCamera) && i < optionData->numberOfOptions - 1)
		{
			i++;
			optionBox.yPos = optionData->OptionYPositions[i] + boxRect->yPos;
		}

		if (MouseOverlappingBox(&optionObj, GameWorld->MainCamera) && i > -1)
		{
			optionData->SelectedOption = clamp(i, 0, optionData->numberOfOptions - 1);
		}

		selectOption = selectOption | MouseClickedObject(&optionObj, GameWorld->MainCamera);
	}
