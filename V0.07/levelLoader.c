...
int loadLevel(World *GameWorld, int level)
{
	...
	if (DebugSettings.DebugTextEnabled == 1)
	{
		printf("\n---------------------------------------");
		printf("\nLoading Level:\n");
	}
...
int loadLevelData(World *GameWorld, int level)
{
	...
	while (endOfFile == 0 && feof(fPtr) == 0)
	{
		...
		if (strcmp(charBuffer, "////") != 0)
		{
			...
			putDebugStrInt("\nLevel ", level);
			putDebugStrInt(" load failed: Data formatted incorrectly! Found at Line: ", lineCount);
		...
		else
		{
			...
			putDebugStrInt("\nUnrecognised data! Found at Line: ", lineCount);
			clearCurrentlyLoadedLevelData(GameWorld);
			GameWorld->GameState = EMPTY_GAME;
			fclose(fPtr);
			return INVALID_DATA;
