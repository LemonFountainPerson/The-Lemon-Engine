...
int RunLemonEngine(void)
{
    ...
    while(GameWorld->GameState != CLOSE_GAME)
    {
	    ...
	    {
	    	RenderSettings.timeSlice = fClamp((float)gameTick/(float)EngineSettings.TickDelta, 0.0, 1.0);
...
int StartUpLemonEngine(void)
{
	...
	FuncResult result = CheckResourceData();
	...
	{
		SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Invalid Data", 
			"Lemon Resource data is missing or corrupt! \nPlease re-install the program.", ScreenData.Window);
		return MISSING_DATA;
	}
	...
	if (LEMON_NETWORKING_ENABLED && !NET_Init())
	{
		SDL_Quit();
		TTF_Quit();
...
FuncResult CheckResourceData(void)
{
	...
	if (!DEBUG_MODE && access(path, W_OK) != -1)
	{
		return INVALID_DATA;
	}
...
int initialiseScreen(RenderFrame *ScreenData, int width, int height, bool Fullscreen)
{
	...
	SDL_CreateWindowAndRenderer(GAME_TITLE, width, height, windowFlag, &ScreenData->Window, &ScreenData->Renderer);
	...
	ScreenData->HUDWidth = ScreenData->screenWidth;
	ScreenData->HUDHeight = ScreenData->screenHeight;
...
// Updates 60 times per second (by default)
...
int GameFrame(World *GameWorld)
{
	...
	updateObjectDisplays(GameWorld);

	#ifdef LEMON_USE_CUSTOM_CALLBACKS
	Update(GameWorld);
	#endif

	updateNetworking(GameWorld);
...
{
	if (INTERPOLATION_ENABLED && DebugSettings.PauseEngine == 0) 	// camera interpolation
	{
		renderCamera.CameraX = renderCamera.prevCameraX + ((renderCamera.CameraX - renderCamera.prevCameraX) * RenderSettings.timeSlice);
		renderCamera.CameraY = renderCamera.prevCameraY + ((renderCamera.CameraY - renderCamera.prevCameraY) * RenderSettings.timeSlice);
	}
...
CameraView* addCameraViewToList(float camX, float camY, int camWidth, int camHeight, float viewPosX, float viewPosY, float width, float height, Layer drawLayer, bool useMain, CameraView list[VIEW_COUNT])
{
	...
	{	
		if (!list[i].active)
		{
			...
			list[i].cam.prevCameraX = camX;
			list[i].cam.prevCameraY = camY;
...
int checkGameFlag(const char name[])
{
	...
	// while (GameFlags[index].name[0] != '\0' && count < GAME_FLAG_COUNT)
	// {
		if (length == GameFlags[index].nameLength && strcmp(GameFlags[index].name, name) == 0)
		{
...
{
	deleteSprite(NULL, EngineSettings.DefaultTexture);
...
float getMouseXHUD()
{
	return MouseInput.xPos * ((float)ScreenData.HUDWidth / (float)ScreenData.screenWidth);
}

float getMouseYHUD()
{
	return MouseInput.yPos * ((float)ScreenData.HUDHeight / (float)ScreenData.screenHeight);
}
...
int SetCameraToHUD(Camera *inputCam)
{
	if (inputCam == NULL)
	{
		return MISSING_DATA;
	...
	ResetCamera(inputCam);
	inputCam->width = ScreenData.HUDWidth;
	inputCam->height = ScreenData.HUDHeight;
	inputCam->zoomedWidth = ScreenData.HUDWidth;
	inputCam->zoomedHeight = ScreenData.HUDHeight;
...
{
	TextSettings.defaultTextPointSize = DEFAULT_TEXT_SIZE;
