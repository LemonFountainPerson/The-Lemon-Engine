...
SpriteSet* createNewSpriteSet(SpriteSetList *setList, int desiredSetID)
{
	...
	{
		putConsoleTS("Creating new spriteset with ID: %d (%s)", desiredSetID, getObjectIDName(desiredSetID));
	}
...
int deleteSprite(SpriteSet *spriteSet, Sprite *input)
{
	...
	if (spriteSet != NULL && spriteSet->lastSprite == input)	// necessary in case this set is a bassis for another
	{
