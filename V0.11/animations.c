...
SpriteSet* loadSpriteSetFromFile(const char FileName[], SpriteSetList *setList, int desiredID)
{
	...
	while (!endOfFile(fPtr))
	{
		...
		if (strcmp(argBuffer, "newanimation:") == 0 || strcmp(argBuffer, "addanimation:") == 0)
		{
			...
			newAnimation = initialiseNewAnimation(inputName, count, frameRate, newSet);
		}
		else if (strcmp(argBuffer, "addframe:") == 0 && newAnimation != NULL)
		{
			// allocates frames as one contiguous chunk for memory optimisation
			...
			// load frames
			for (int i = 0; i < newAnimation->frameCount; i++)
			{
				if (i != 0)
				{
					getNextArg(fPtr, argBuffer, MAX_LEN);
...
void loadFrameSound(FILE *fPtr, AnimationFrame *newFrame, Animation *anim)
{
	...
	if (strcmp(buffer, "playsound:") == 0)
	{
		...
		newFrame->soundIndex = addSoundToAnimation(buffer, volume, channel, anim);
	}
	else
	{
		fseek(fPtr, filePos, SEEK_SET);
...
Animation* getAnimationByIndex(int index, DisplayData *inputData)
{
	if (inputData == NULL || inputData->spriteSetSource == NULL)
	{
		return NULL;
...
int PlayAnimationByIndex(int index, int loopCount, DisplayData *inputData)
{
	...
	return useThisAnimation(getAnimationByIndex(index, inputData), loopCount, inputData);
}
...
int iterateAnimation(DisplayData *inputData, float deltaTime)
{
	...
	// If value is started on <= 0, then on animation end value will be less than 0, and it will loop indefinitely
	if (inputData == NULL || inputData->currentAnimation < 1)
	{
		return ACTION_DISABLED;
	}
	
	// animation pointer not set but ID is, check if ID corresponds to real animation
	if (inputData->animationBuffer == NULL)
	{
		inputData->animationBuffer = getAnimationByIndex(inputData->currentAnimation, inputData);
	}
	...
	// frameBuffer pointer is not set but currentFrame is, check if corresponds to real frame
	if (inputData->frameBuffer == NULL)
	{
		if (inputData->currentFrame < 0 || inputData->currentFrame >= anim->frameCount)
		{
			inputData->currentFrame = 0;
		}

		inputData->frameBuffer = &anim->animationData[inputData->currentFrame];
	}
	...
	while (inputData->animationTick >= frameRate)
	{
		...
		if (inputData->currentFrame >= anim->frameCount - 1)
		{
...
Animation* initialiseNewAnimation(const char animationName[], int frameCount, float frameRate, SpriteSet *inputSet)
{
	if (inputSet == NULL)
	{
	...
	memset(newAnimation->animationData, 0, sizeof(AnimationFrame) * frameCount);
	...
	LemonStrncpy(newAnimation->name, animationName, MAX_LEN);
...
int addSoundToAnimation(const char name[], float volume, int channel, Animation *anim)
{
	...
	{
		SoundMeta *list = anim->animationSounds;
		for (int i = 0; i < anim->soundCount; i++)
		{
			if (strcmp(list[i].soundName, name) == 0 && list[i].channel == channel && fabs(list[i].volume - volume) < 0.01)
			{
				return i;
			}
		}
...
{
	if (sourceSet == NULL || inputAnimation == NULL || spriteName == NULL || inputAnimation->animationData == NULL)
	{
		return NULL;
	}
	...
	AnimationFrame *list = inputAnimation->animationData;
	AnimationFrame *newFrame = NULL;

	// look for an empty pre-allocated slot
	for (int i = 0; i < inputAnimation->frameCount; i++)
	{
		if (list[i].frameSprite == NULL)
		{
			newFrame = &list[i];
			i = inputAnimation->frameCount;
		}
	}
