...
const Uint32 colourMap[8] = {	0x00FF00FF, 0x00FF1010, 0x00FF1010, 0x00FFAA00, 
							 	0x00F8F800, 0x000010FF, 0x00AAFF10, 0x0000DDDD};
...
void enableMenuCamera(Camera *input, float xPos, float yPos)
{
	// if already on menu state, dont repeat steps
	if (input->CameraMode == MENU_CAMERA)
	{
		return;
	}

	input->CameraMode = MENU_CAMERA;
	input->CameraXBuffer = input->CameraX;
	input->CameraYBuffer = input->CameraY;

	input->CameraX = xPos;
	input->CameraY = yPos;
	input->prevCameraX = input->CameraX;
	input->prevCameraY = input->CameraY;

	return;
}

void disableMenuCamera(Camera *input, CameraState camMode)
{
	// if not on menu state, return
	if (input->CameraMode != MENU_CAMERA)
	{
		return;
	}

	input->CameraMode = camMode;
	input->CameraX = input->CameraXBuffer;
	input->CameraY = input->CameraYBuffer;
	input->prevCameraX = input->CameraX;
	input->prevCameraY = input->CameraY;
	...
	{	
		...
		if (drawLayer == HUD)
		{
			...
			while(currentObject != NULL)
			{
				...
				{
					renderObject(hudCam, currentObject, Screen, GameWorld);
				}
		...
		else
		{
			...
			while(currentObject != NULL)
			{
				...
				if (drawLayer == objLayer)
				{
					renderObject(inputCamera, currentObject, Screen, GameWorld);
				}
...
int renderHitbox(Camera inputCamera, PhysicsBox *inputBox, SDL_Renderer *Screen)
{
	...
	uint32_t colour = colourMap[inputBox->solid % 8];
	...
	if (RenderSettings.HitboxThickness > 50)
	{
	...
	Hitbox.w = (float)clamp(RenderSettings.HitboxThickness, 0, inputBox->xSize);

	SDL_RenderFillRect(Screen, &Hitbox);

	Hitbox.x += (float)(inputBox->xSize - clamp(RenderSettings.HitboxThickness, 0, inputBox->xSize));
	SDL_RenderFillRect(Screen, &Hitbox);

	Hitbox.x = xCoord;
	Hitbox.w = (float)inputBox->xSize;
	Hitbox.h = (float)clamp(RenderSettings.HitboxThickness, 0, inputBox->ySize);
	SDL_RenderFillRect(Screen, &Hitbox);

	Hitbox.y += (float)(inputBox->ySize - clamp(RenderSettings.HitboxThickness, 0, inputBox->ySize));
	SDL_RenderFillRect(Screen, &Hitbox);
...
int renderObject(Camera inputCamera, Object *input, SDL_Renderer *Screen, World *GameWorld)
{
	...
	DisplayData inputData = (*input->ObjectDisplay);
	...
	{
		if (inputBox.xPos != inputBox.prevXPos)
		{
			inputBox.xPos = inputBox.prevXPos + ((inputBox.xPos - inputBox.prevXPos) * RenderSettings.timeSlice);
		}

		if (inputBox.yPos != inputBox.prevYPos)
		{			
			inputBox.yPos = inputBox.prevYPos + ((inputBox.yPos - inputBox.prevYPos) * RenderSettings.timeSlice);		
		}
	}
	...
	Polygon *polygon = getPolygon(input, GameWorld);
	...
	if (inputRenderMode == TILE || inputRenderMode == TILE_FAST)
	{
		renderTiledSprite(Screen, inputCamera, inputData, &renderBox, getTileMap(input, GameWorld));
	}
...
int renderTiledSprite(SDL_Renderer *Screen, Camera inputCamera, DisplayData inputData, SDL_FRect *renderBox, TileMap *map)
{
	...
	if (map == NULL || sprite->width < ((int)map->tileSize * 3) || sprite->height < ((int)map->tileSize * 3))
	{
...
{
	if (GameWorld == NULL || DebugSettings.DebugTextDisplayMode == DEBUG_TEXT_DISABLED)
	{
		return;
	}
	...
	switch(DebugSettings.DebugTextDisplayMode)
	{
		...
		case DEBUG_TEXT_SINGLE_OBJECT:
			...
			if (MouseInput.RightButton == 1)
			{
				...
				deleteObject(currentObject, &GameWorld->ObjectList);
			}
			break;
...
Text* addDebugText(const char inputPhrase[], float x, float y, int wrapwidth, DebugTextFormatting format)
{
	...
	int index = 0;
	while (index < MAX_TEXTS && TextsArray[index].text != NULL)
	{  
		index++;
	}
